...
/**
 * Posts reportable exceptions to a Discord channel via an incoming webhook
 * (FOUNDATION: alerts/diagnostics go to a Discord channel) as a rich embed.
 * No-ops when no webhook is configured, and never lets a delivery failure
 * escape — that would loop back into the exception handler.
 */
class DiscordErrorReporter
{
    ...
    private function trace(Throwable $e): string
    {
        ...
        return $lines === [] ? '(brak śladu stosu)' : implode("\n", $lines);
    }
